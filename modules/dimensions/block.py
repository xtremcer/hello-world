...
def analyze_block(stock_code: str, industry: str = "", main_type: str = "短线游资") -> Tuple[str, Dict]:
    ...
    # 如果没有配置行业，尝试自动获取
    if not industry:
        print(f"  行业未配置，尝试通过baostock自动获取...")
        auto_industry, error = get_stock_industry(stock_code)
        if auto_industry is not None and auto_industry.strip():
            industry = auto_industry.strip()
            print(f"  ✅ 自动获取行业成功：{industry}")
        else:
            stock_md_content += f"- ❌ **数据缺失**：行业信息未配置且自动获取失败：{error if error else '未知错误'}\n"
            stock_md_content += "- 说明：请在STOCK_LIST中配置industry字段\n"
            stock_md_content += "- 信号判定：**无法判断**\n\n"
            json_data = {
                "error": f"行业信息未配置且自动获取失败：{error if error else '未知错误'}",
                "qualified": None,
                "source": "缺失"
            }
