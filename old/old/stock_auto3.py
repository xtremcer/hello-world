...
def get_stock_data(stock_code, stock_name="", main_type="短线游资", industry="", target_date=None):
    ...
    if fund_df is not None and not fund_df.empty:
        ...
        for col in fund_data.columns:
            ...
            if '净流入' in col or 'net' in col_lower or '净额' in col:
                net_inflow_col = col
            if '比例' in col and ('成交' in col or 'amount' in col_lower or 'ratio' in col_lower):
                ratio_col = col

        # 如果找不到标准列名，尝试使用其他可能的列名
        if net_inflow_col is None:
            # 打印实际列名用于调试
            print(f"  ⚠️ 未找到净流入列，实际列名：{list(fund_data.columns)}")
            stock_md_content += f"- ❌ **数据列名不匹配**：无法找到净流入相关列\n"
            stock_md_content += f"- 实际列名：{', '.join([str(c) for c in fund_data.columns])}\n"
            stock_md_content += "- 说明：数据接口列名可能已变更\n"
            stock_md_content += "- 信号判定：**无法判断**\n\n"
            ...
            try:
                ...
                if ratio_col:
                    avg_ratio = fund_data[ratio_col].mean()
                else:
                    avg_ratio = 0
                    print(f"  ⚠️ 未找到比例列，实际列名：{list(fund_data.columns)}")

                stock_md_content += f"- 近3日主力资金净流入总额：{round(net_inflow, 2)}万元\n"
                if ratio_col:
                    stock_md_content += f"- 净流入占成交额平均比例：{round(avg_ratio, 2)}%\n"
                inflow_qualified = net_inflow > 0
                ratio_qualified = avg_ratio > 5 if ratio_col else False
                ...
                json_data["analysis"]["fund_flow"] = {
                    "net_inflow_3d": round(float(net_inflow), 2),
                    "avg_ratio_3d": round(float(avg_ratio), 2) if ratio_col else 0,
                    "qualified": bool(inflow_qualified and ratio_qualified),
                    "detail_5d": fund_df.iloc[-5:].round(2).to_dict('records'),
                    "source": "akshare"
                }
            except Exception as e:
                print(f"  ⚠️ 处理主力资金数据失败：{str(e)}")
                stock_md_content += f"- ❌ **数据处理失败**：{str(e)}\n"
                stock_md_content += "- 信号判定：**无法判断**\n\n"
                json_data["analysis"]["fund_flow"] = {
                    "error": str(e),
                    "qualified": None,
